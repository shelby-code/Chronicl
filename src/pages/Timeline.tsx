...
interface TimelineEvent {
  ...
  sourceName: string
  sourceId: string
  sourceType: 'civilization' | 'country'
  color: string
  flag?: string
}
...
type Filter = 'all' | 'civilizations' | 'countries'

export default function Timeline() {
  const [filter, setFilter] = useState<Filter>('all')

  const civEvents: TimelineEvent[] = civilizations.flatMap((c) =>
    c.timeline.map((e) => ({
      year: e.year,
      event: e.event,
      sourceName: c.name,
      sourceId: c.id,
      sourceType: 'civilization' as const,
      color: (c as any).color ?? '#D4A017',
    }))
  )

  const countryEvents: TimelineEvent[] = (countries as any[]).flatMap((c) =>
  ...
  const allEvents = [...civEvents, ...countryEvents]
    .sort((a, b) => parseYear(a.year) - parseYear(b.year))

  const visible = filter === 'all' ? allEvents
    : filter === 'civilizations' ? civEvents.slice().sort((a, b) => parseYear(a.year) - parseYear(b.year))
    : countryEvents.slice().sort((a, b) => parseYear(a.year) - parseYear(b.year))
  ...
  return (
    <>
      <SEO
        ...
        description={`${allEvents.length} historical events across ${civilizations.length} civilizations and ${countries.length} countries, sorted chronologically from the ancient world to the modern age.`}
        path="/timeline"
      />
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mb-10 fade-up-1">
            ...
            <p className="text-[#9ca3af] text-base">
              {allEvents.length} events across {civilizations.length} civilizations and {countries.length} countries — from the first cities to today.
            </p>
          </div>

          {/* Filter tabs */}
          <div className="flex gap-2 mb-10 fade-up-2">
            {([
              { key: 'all', label: 'All Events', count: allEvents.length },
              { key: 'civilizations', label: 'Civilizations', count: civEvents.length },
              { key: 'countries', label: 'Countries', count: countryEvents.length },
            ] as { key: Filter; label: string; count: number }[]).map(({ key, label, count }) => (
              <button
                key={key}
                onClick={() => setFilter(key)}
                className="flex items-center gap-2 px-4 py-2 rounded-full text-[12px] font-medium transition-all duration-150 whitespace-nowrap"
                style={filter === key ? {
                  ...
                  return (
                    <div key={`${item.sourceId}-${i}`}>
                      ...
                      <div className="relative group">
                        ...
                        >
                          <p className="text-[11px] font-bold mb-1.5 tracking-wide" style={{ color: item.color }}>
                            {item.year}
                          </p>
                          <p className="text-[13.5px] text-[#d0d0d0] leading-snug mb-2">{item.event}</p>
                          <Link
                            to={`/${item.sourceType === 'civilization' ? 'civilization' : 'country'}/${item.sourceId}`}
                            className="inline-flex items-center gap-1.5 text-[11px] font-medium text-[#9ca3af]/50 hover:text-[#D4A017] transition-colors"
                          >
